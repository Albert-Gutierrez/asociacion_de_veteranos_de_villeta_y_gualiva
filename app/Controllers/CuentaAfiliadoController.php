...
class CuentaAfiliadoController
{
    ...
    /**
     * Único dato que el propio afiliado puede cambiar desde "Mi información".
     * Se guarda en img/perfiles/ (no es información sensible).
     */
    public function subirFoto(): void
    {
        ...
        $asociadoActual = $modelo->buscarPorId($afiliado['id']);
        $fotoAnterior = $asociadoActual['foto_ruta'] ?? null;
        if ($fotoAnterior && is_file($carpetaDestino . $fotoAnterior)) {
            unlink($carpetaDestino . $fotoAnterior);
        }

        $modelo->actualizarFoto($afiliado['id'], $nombreArchivo);
